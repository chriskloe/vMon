...
public class Vmonitor implements TelnetNotificationHandler
{
    ...
    public static void main(String[] args) throws Exception
    {
        if(args.length < 3)
        {
            System.err.println("Usage: Vmonitor <-u <username> <-p<password>>> <vctrl remote-ip> <vctrl remote-port>"
            		+ "<openhab rest uri>");
            System.exit(1);
        }
        ...
        for( int idx = 0; idx != args.length; ++idx )
        {
        	...
        	case 0:
        		...
        		break;
        	case 1:	
        		remoteport = Integer.parseInt( args[ idx ], 10 );
        		++argsRead;
        		break;
        	case 2:
        		openhabUri = args[ idx ];
    		...
    		try
            {
	            ...
	            while (true)
	            {
	                for (VCommand vc :mCommands )
	                {
	                	...
	                	RestInterface.sendPostRequest(openhabUri,vc.getmName(),vc.getmValue(), username, password );
	                }
	            	if (mReconnect) break;
	            	Thread.sleep(5000);
	            }//inner while loop
